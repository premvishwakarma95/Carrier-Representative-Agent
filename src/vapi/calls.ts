...
export async function createOutboundCall(params: {
  ...
  // nova-3 keyterm boost (see src/assistant/create.ts's transcriber config)
  // so THIS specific name transcribes more reliably if the same contact
  // says it again on this call. Deliberately per-call and dynamic, not a
  // static list — we have no fixed set of expected names to pre-register,
  // only ever the one specific name we already have on file for this one
  // carrier. Omit when there's no known contact yet (first-time calls have
  // nothing to boost toward).
