 ...
 * MDR's own contact_name field — confirmed empirically (2026-09-11) that a
 * successful updateCarrierDetail write does not come back through
 * getSpecificCarrier/getAllCarriers, so MDR's copy can't be trusted as the
 * "did we already confirm this" signal. This local record is the only
 ...
 * current dial (in_progress, no confirmedContactName yet) — same defensive
 * pattern the old callMemory.ts used, to avoid a genuinely first-ever call
 * seeing its own not-yet-happened attempt reflected back at it.
 */
