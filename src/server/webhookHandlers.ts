...
async function scheduleCallback(params: any, { attempt }: CallContext) {
  ...
  // explicit instruction) — the carrier is the one naming this time, which
  // is a stronger signal than our default calling-window policy exists to
  // protect: never tell them it's outside our hours or ask for a different
  // time, just accept whatever they said. dispatch.ts's matching change
  // (see its scheduledFor comment) is what actually dials at this exact
  // time later, unchecked.
