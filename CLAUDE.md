...
```
The same logic also runs on a `node-cron` schedule inside `src/server/index.ts` — both the route and the cron job call the same exported `runDispatchCycle()`, one implementation. The cron job is **active** (`*/5 * * * *`, every 5 minutes, plus once immediately on process startup) — confirmed live on staging 2026-09-24. The manual route above still works too, for one-off testing (e.g. `?dryRun=true` to preview without side effects).
...
5. **`src/server/`** — the whole decision + webhook layer, one Express app:
   ...
   - `dispatch.ts` — `POST /dispatch/run` (optional `?dryRun=true`) plus the exported `runDispatchCycle(dryRun)`. For every open local `Load` and every non-`stop_call` local `Carrier`: re-fetches fresh MDR carrier state, checks close/threshold/stop/timezone/`email_sent_at` validity, computes the next due attempt via `cadence.ts`, checks `callingWindow.ts`, creates a `CallAttempt` (idempotent — a duplicate-key race loser is treated as "already claimed," not an error), and calls `createOutboundCall`. If the prior `CallAttempt` has `callbackAt` set (from `schedule_callback`), that exact time is used as `scheduledFor` instead of the normal cadence math, **and the calling-window check is skipped entirely for that one attempt** (2026-09-24, explicit instruction) — a carrier naming their own availability is treated as pre-approved, dialed at exactly that time regardless of the configured window. Every other, cadence-computed attempt is still fully window-gated as before.
   - `cadence.ts` — the confirmed attempt cadence as pure functions (attempt 1: next calling-window open, as soon as the carrier is known to us — the earlier 30-min-after-`email_sent_at` gate was removed 2026-08-20, since MDR's push-based integration means the timing was already decided by the time we see the carrier; `email_sent_at` is still validated elsewhere in `dispatch.ts` as a data-quality check, just no longer used for attempt-1 timing; attempt 2: +1hr; attempt 3: +2hr; attempt 4: next business morning). `MAX_CALL_ATTEMPTS = 4`.
   - `callingWindow.ts` — timezone-aware calling-hours logic: `isWithinCallingWindow`, `nextCallingWindowOpen`, `nextBusinessMorning`, `wallClockToUtc` (interprets an LLM-supplied wall-clock callback time against the carrier's real MDR-sourced timezone, ignoring any offset embedded in the model's string — MDR's value is always the source of truth, never a carrier-stated one), `formatCallingWindow`. `nextCallingWindowOpen`/`nextBusinessMorning` walk day-by-day and always snap to an exact `CALLING_WINDOW_START_HOUR:00` local (2026-09-24 fix) — the earlier version stepped forward in fixed 30-minute increments from the original target timestamp, so a due time like 4:40 PM Monday landed at 8:10/8:17 AM Tuesday instead of a clean 8:00; also fixed `nextBusinessMorning` (cadence attempt 4) always advancing at least one full calendar day regardless of window width, since under a 24-hour window it used to trivially short-circuit to the same day.
   ...
   - `index.ts` — mounts `mdrWebhook.ts` at `/webhooks/mdr` and `dispatch.ts` at `/dispatch`, defines `POST /vapi/tool-calls` and `GET /health`, plus the active cron registration (see Commands above).
...
| `schedule_callback` | local only — MDR has no callback endpoint; writes `CallAttempt.callbackAt`/`callResult`. Not gated by the calling window (2026-09-24) — a carrier-proposed time is accepted as-is, even outside configured hours; see `dispatch.ts` above for the matching dial-time bypass |
